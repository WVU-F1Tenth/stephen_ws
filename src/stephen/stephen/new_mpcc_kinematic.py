...
class mpc_config:
    ...
    )  # input cost matrix, penalty for inputs - [v, steering]
    Rdk: npt.NDArray[Any] = field(
        default_factory=lambda: np.diag([0.01, 80.0])
    )  # input difference cost matrix, penalty for change of inputs - [v, steering]
...
class MPC(Node):
    ...
    def __init__(self):
        ...
        self.ov_cmd = [0.0] * self.config.TK
    ...
    def pose_callback(self, odometry_info):
        ...
        ref_path = self.calc_ref_trajectory(vehicle_state, self.ref_x, self.ref_y, self.ref_yaw, self.ref_v, self.ov_cmd)
        ...
        (
            ...
            oyaw,
            state_predict,
        ) = self.linear_mpc_control(ref_path, x0, self.ov_cmd, self.odelta)
        
        self.ov_input = np.clip(self.ov_cmd[0], # type: ignore
    ...
    def mpc_prob_init(self):
        ...
        constraints.append(self.xk[2, 1:] == self.uk[0, :])
    ...
    def update_state(self, state, v_cmd, delta):
        delta = np.clip(delta, -self.config.MAX_STEER, self.config.MAX_STEER)
        v_cmd = np.clip(v_cmd, self.config.MIN_SPEED, self.config.MAX_SPEED)
        state.x += v_cmd * math.cos(state.yaw) * self.config.DTK
        state.y += v_cmd * math.sin(state.yaw) * self.config.DTK
        state.yaw += (v_cmd / self.config.WB) * math.tan(delta) * self.config.DTK
        state.v = v_cmd
        return state
    ...
    def get_model_matrix(self, v, phi, delta):
        ...
        # v_cmd input
        B[0, 0] = self.config.DTK * math.cos(phi)
        B[1, 0] = self.config.DTK * math.sin(phi)
        B[2, 0] = 1.0
        B[3, 0] = self.config.DTK * math.tan(delta) / self.config.WB
        # steering input
        B[3, 1] = self.config.DTK * v / (
            self.config.WB * math.cos(delta) ** 2
        )
        C = np.zeros(self.config.NXK)
        C[0] = self.config.DTK * v * math.sin(phi) * phi
        C[1] = -self.config.DTK * v * math.cos(phi) * phi
        C[3] = -self.config.DTK * v * delta / (
            self.config.WB * math.cos(delta) ** 2
        )
        return A, B, C

    def mpc_prob_solve(self, ref_traj, path_predict, x0, ov_cmd, odelta):
        self.x0k.value = x0
        A_block = []
        B_block = []
        C_block = []
        for t in range(self.config.TK):
            A, B, C = self.get_model_matrix(
                path_predict[2, t], path_predict[3, t], odelta[t] #type: ignore
            )
            A_block.append(A)
            B_block.append(B)
            C_block.extend(C)
        ...
        self.prev_u_k.value = np.array([ov_cmd[0], odelta[0]])
        ...
        if self.mpc_solve_time > 0.025:
            print(f'\nMPC SOLVE TIME OF {self.mpc_solve_time:.3f}')
        ...
        if (
            ...
            ov_cmd = np.array(self.uk.value[0, :]).flatten()
            odelta = np.array(self.uk.value[1, :]).flatten()

        else:
            print("Error: Cannot solve mpc..")
            ov_cmd, odelta, ox, oy, oyaw, ov = None, None, None, None, None, None

        return ov_cmd, odelta, ox, oy, ov, oyaw

    def linear_mpc_control(self, ref_path, x0, ov_cmd, odelta):
        """
        ...
        if ov_cmd is None or odelta is None:
            ov_cmd = [0.0] * self.config.TK
            odelta = [0.0] * self.config.TK

        # Call the Motion Prediction function: Predict the vehicle motion for x-steps
        path_predict = self.predict_motion(x0, ov_cmd, odelta, ref_path)

        # Run the MPC optimization: Create and solve the optimization problem
        mpc_a, mpc_delta, mpc_x, mpc_y, mpc_v, mpc_yaw = self.mpc_prob_solve(
            ref_path, path_predict, x0, ov_cmd, odelta)
