...
# Track problem features:
# - Wrong path: A path that is not a dead end, but is incorrect,
#      such as the open ended long straight on strata_basement.
# - Solution: Use heuristic such as preferring left-most (left for counter clockwise) path if available.

# - Dead end: A open section that has no path (although this may not be known immediately).
# - Solution: Require chosen gap include possible path (represented by disparity).

# - Fov too small: At the least this causes a delay in finding the right path only turns close to 180 degrees.
# - Solution: Widen fov if no path found (represented by disparity)
...
# TODO:
# - Oscilates when prefered choice is dead end, but looks like disparity as it drives away
# - Logic to take wide turns
# - Implement min frames before path change
# - Negative curves increase minimum disparity extension needed (explore alternative solutions)
# - CREATE 2 PROGRAM OUTPUT CHECKING FOR VALIDATING REFACTORING
...
class PathFollow(Node):
    ...
    def __init__(self):
        ...
        self.path = Path(
            ...
            speed_depth_extension=self.car_radius,
            min_gap_width=0.0,
    ...
    def adjust(self, scan: LaserScan):
        ...
        time = self.time_start - perf_counter()
...
class Path:
    def __init__(self, disparity_threshold, extension, steering_extension, speed_depth_extension,
                  min_gap_width, track_direction, max_steering_angle):
    ...
    def disparity_extend(self, ranges):
        ...
        pos_disp = np.flatnonzero(diffs >= self.disparity_threshold)
        neg_disp = np.flatnonzero(diffs <= -self.disparity_threshold)
    ...
    def disparities(self, ranges):
        diffs = np.diff(ranges)
        return (np.flatnonzero(diffs >= self.disparity_threshold),
        np.flatnonzero(diffs <= -self.disparity_threshold))
    ...
    def steering(self, start, width, depth):
        if width > 0: sign = 1
        elif width < 0: sign = -1
